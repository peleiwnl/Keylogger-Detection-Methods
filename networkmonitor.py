...
class KeyloggerDetector:
    def __init__(self, root):
        ...
        # Timer to update process list
        self.update_process_list()
        self.root.after(10000, self.update_process_timer)  # Update every 10 seconds
    
    def update_process_timer(self):
        """Timer function to update process list periodically"""
        self.update_process_list()
    ...
    def setup_ip_port_monitor_tab(self):
        ...
        ttk.Label(interface_frame, text="Network Interface:").pack(side=tk.LEFT, padx=5)
        self.ip_interface_var = StringVar(value="")  # Default value
    ...
    def select_process(self, event):
        ...
        if selected_item:
            ...
            self.tabs.select(0)  # Switch to process monitor tab
    ...
    def process_monitor_worker(self, pid, duration):
        """Worker function to monitor a process for connections"""
        try:
            process = psutil.Process(pid)
            process_name = process.name()
            
            # Since we're in a separate thread, we need to use the after method to update the GUI
            ...
            while time.time() - start_time < duration and not stop_flag and self.is_monitoring:
                try:
                    ...
                    # Filter for our process
                    for conn in system_connections:
                        if conn.pid == pid and conn.raddr:  # Only consider connections with remote addresses
            ...
            self.root.after(0, lambda: self.log_message("Try running the application with administrator privileges."))
            self.root.after(0, self.monitoring_finished)
        except Exception as e:
    ...
    def table_click_handler(self, event):
        ...
        # Only respond to clicks on the third column (action column)
        if column == '#3':
    ...
    def monitor_connection(self, remote_addr):
        ...
        try:
            ...
            self.tabs.select(1)  # Switch to IP/Port monitor tab
        except Exception as e:
    ...
    def start_ip_port_monitoring(self):
        ...
        try:
            ...
            # Create and start the worker thread
            self.monitor_thread = threading.Thread(
                target=self.ip_port_monitor_worker,
    ...
    def ip_port_monitor_worker(self, target_ip, target_port, interface):
        """Worker function to monitor specific IP/port traffic"""
        # Keep track of alert information
        ALERT_INTERVAL = 300  # How often to allow alerts for the same source (seconds)
        
        # Initialize connection data dictionary
        connection_data = {}
                ...
                if Raw in packet:
                    ...
                    if b"POST" in payload:
                        ...
                        if ip_key not in connection_data:
                            ...
                            # check for patterns (need at least 4 timestamps)
                            if len(data["timestamps"]) >= 4:
                                ...
                                # Calculate statistical measures of the intervals
                                ...
                                # Calculate coefficient of variation (CV) - lower values indicate more consistency
                                cv = (std_dev / mean_interval) if mean_interval > 0 else float('inf')
                                
                                # Check if this is the first alert or if we've received 4 more timestamps since last alert
                                show_alert = False
                                ...
                                # Threshold for regularity - lower CV means more regular intervals
                                # For automated processes like keyloggers, CV is typically very low (<0.1)
                                if cv < 0.2 and len(intervals) >= 3 and show_alert:
                                    ...
                                    self.root.after(0, lambda: self.log_message(
                                        f"Regularity: {cv:.4f} coefficient of variation (lower is more suspicious)", True))
        
        try:
            # Use a specific filter for better performance
            # Modified filter to specifically look for HTTP traffic
    ...
    def stop_monitoring(self):
        ...
        if self.is_monitoring:
            self.is_monitoring = False
            self.stop_sniffing = True  # Signal the packet sniffer to stop
            self.log_message("Stopping monitoring...", self.tabs.index("current") == 1)
            
            # The monitoring_finished method will be called by the worker thread
    ...
    def monitoring_finished(self):
        ...
        # Update buttons
        if self.tabs.index("current") == 0:  # Process Monitor tab
            self.monitor_btn.config(text="Start Monitoring Process Connections", 
                                  command=self.start_process_monitoring)
        else:  # IP/Port Monitor tab
